...
export default function About() {
  ...
  const { heading = 'About Mystry Signage', body, mission, image } = data || {};
        ...
        <div className="container-page grid grid-cols-1 items-center gap-12 md:grid-cols-2">
          <div>
            ...
            <h1 className="font-display text-3xl text-paper sm:text-4xl">
              <span className="tube-underline text-amber text-glow-amber">{heading}</span>
            </h1>
            {body && <p className="mt-6 whitespace-pre-line leading-relaxed text-ash">{body}</p>}
            {mission && (
          ...
          <div className="aspect-[4/3] overflow-hidden rounded-2xl border border-ash/15 bg-panel">
            {image ? (
              <img src={resolveUrl(image)} alt="Mystry Signage team at work" className="h-full w-full object-cover" />
            ) : (
              <div className="flex h-full items-center justify-center text-ash">Add a photo from the admin dashboard</div>
            )}
          </div>
        </div>
      </section>
