      ...
      <div className="container-page relative grid grid-cols-1 items-center gap-12 py-20 md:grid-cols-2 md:py-28">
        <div>
          ...
          <h1 className="font-display text-4xl leading-tight text-paper sm:text-5xl lg:text-6xl">
            {heading.split(' ').map((word, i) =>
              i === 0 ? (
                <span key={i} className="tube-underline text-amber text-glow-amber">
                  {word}{' '}
                </span>
              ) : (
          ...
          <p className="mt-6 max-w-lg text-base leading-relaxed text-ash sm:text-lg">{subheading}</p>
        ...
        <div className="relative">
          <div className="aspect-[4/3] w-full overflow-hidden rounded-2xl border border-ash/15 bg-panel shadow-neonAmber">
            {image ? (
              <img src={resolveUrl(image)} alt="Featured signage project" className="h-full w-full object-cover" />
            ) : (
              <div className="flex h-full w-full items-center justify-center">
                <span className="font-display text-3xl text-amber/70 text-glow-amber">MYSTRY SIGNAGE</span>
              </div>
            )}
          </div>
        </div>
